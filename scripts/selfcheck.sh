...
UNIT_OWNED=0
if systemctl --user is-active --quiet cursor-magic; then
  UNIT_OWNED=1
  systemctl --user restart cursor-magic   # new MainPID rewrites magic.pid
else
...
while time.time() - t0 < 4.0:
    ...
    time.sleep(0.04)
PY
sleep 0.5; echo "restored live: $(cat $HOME/.cache/cursor-magic/magic.pid) alive=$(kill -0 $(cat $HOME/.cache/cursor-magic/magic.pid) 2>/dev/null && echo yes)"